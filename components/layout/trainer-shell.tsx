...
export function TrainerShell({ children, staff }: Props) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between px-4 h-14 border-b border-sidebar-border bg-sidebar/80 backdrop-blur-md">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-bold text-foreground leading-none">{staff.gym?.name ?? "Pulse"}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide font-medium">Trainer Portal</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:block">
              <p className="text-xs font-medium text-foreground leading-none">{staff.full_name}</p>
              <p className="text-[10px] text-muted-foreground mt-0.5 flex items-center gap-1">
                <Dumbbell className="w-2.5 h-2.5" /> Trainer
              </p>
            </div>
          </div>
          <button

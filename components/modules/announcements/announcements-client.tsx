...
export function AnnouncementsClient({ gymId, announcements: initial }: Props) {
  ...
  async function reload() {
    ...
    const { data } = await supabase.from("pulse_announcements")
      .select("*").eq("gym_id", gymId)
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const supabase = createClient();
    const { error } = await supabase.from("pulse_announcements").insert({
      gym_id: gymId,
      title: form.title,
  ...
  async function togglePin(a: Announcement) {
    const supabase = createClient();
    const { error } = await supabase.from("pulse_announcements").update({ is_pinned: !a.is_pinned }).eq("id", a.id);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Gym notices & updates</p>
        </div>
      ...
      {announcements.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-2 text-muted-foreground rounded-2xl border border-sidebar-border bg-card">
          ...
          <p className="text-xs">Post an announcement for your gym members</p>
        </div>
      )}
      ...
      {pinned.length > 0 && (
        <div className="space-y-3">
          ...
          {pinned.map((a) => (
            <div key={a.id} className="rounded-2xl border border-primary/20 bg-primary/5 p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-primary hover:bg-primary/10" onClick={() => togglePin(a)}>
          ...
          {rest.map((a) => (
            <div key={a.id} className="rounded-2xl border border-sidebar-border bg-card p-5 hover:border-white/10 transition-colors">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary hover:bg-primary/10" onClick={() => togglePin(a)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-2">
            <div className="space-y-1.5">
              <Label>Title *</Label>
              <Input placeholder="e.g. Gym closed on Friday" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <Label>Content *</Label>
              <Textarea placeholder="Write the announcement…" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={4} />
            </div>
            <label className="flex items-center gap-3 cursor-pointer">
              <div
                className={`w-10 h-5 rounded-full border transition-colors relative ${form.is_pinned ? "bg-primary border-primary/50" : "bg-white/10 border-white/20"}`}

  ...
  return (
    <div className="rounded-xl border border-sidebar-border bg-card px-3 py-2 shadow-xl text-xs">
      ...
      <p className="text-muted-foreground">
        Collection rate: <span className="font-medium text-foreground">{payload[0].value}%</span>
      </p>
    </div>
  );
}
      ...
      <BarChart data={data} margin={{ top: 16, right: 4, left: 0, bottom: 0 }}>
        ...
        <Bar dataKey="collectionRate" name="Collection Rate" radius={[6, 6, 0, 0]}>
          <LabelList
            ...
            position="top"
            style={{ fontSize: 10, fill: "hsl(220 18% 60%)" }}
            formatter={(v: number) => v > 0 ? `${v}%` : ""}
          />
          {data.map((entry) => (
            <Cell
              key={entry.monthKey}
              fill={entry.collectionRate >= 80 ? "#10b981" : entry.collectionRate >= 50 ? "#0066ff" : "#f43f5e"}
              fillOpacity={0.7}
            />
          ))}
        </Bar>
